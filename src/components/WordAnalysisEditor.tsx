...
export function WordAnalysisEditor(){
    ...
    return(
        ...
        <NodeEditorOption my_label="Word to analyze:" my_value_input="" disabled = {Model.first_step_performed.value} onInputHandler={(event)=>Model.updateWordToAnalyze(event)}></NodeEditorOption>
        <div class = "flex flex-col box-border gap-[10px]">
        <label class = "pl-[40px]">{Model.auto_word_resolution.value}</label>
        <button disabled = {Model.exists_default_transition.value || Model.first_step_performed} onClick = {()=>Model.computeWordDirectly()}> Auto computation</button>
        ...
        {Model.first_step_performed.value ? <div class = "flex box-border flex-col"><label class = "block whitespace-pre-line">{Model.step_by_step_word_resolution.value}</label>{ valid_first_step ? <button onClick = {Model.computeStepByStep}>Next Step</button> : null} </div> : null}

...
export function NodeEditor(){
    ...
    selected_node_data = Model.findSelectedCredentials();
    return(
        ...
        <NodeStateChange my_class = "gap-[12px]" my_label = "Ending node" my_value_input = {selected_node_data.final_node} onInputHandler={(event)=>Model.changeFinalNodeStatus(event,selected_node_data.id)}></NodeStateChange>
        </div>
        <label>Connections: </label>
        {selected_node_data.connections.map((connection)=>{
            let my_string = selected_node_data.name + "-> " + connection.ending_name;
            ...
            <button class = "grow" onClick = {()=>Model.deleteConnection(selected_node_data.id,connection)}>Delete</button></div>);
        })}
